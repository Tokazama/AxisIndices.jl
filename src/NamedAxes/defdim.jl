...
macro defdim(
    ...
    esc(quote
        ...
        if $def_naxis
            NamedAxes.@def_naxis($name, $name_dim)
        end

        if $def_axis_keys
            NamedAxes.@def_axis_keys($name, $name_dim)
        end

        if $def_axis_indices
            NamedAxes.@def_axis_indices($name, $name_dim)
        end

        if $def_axis_type
            NamedAxes.@def_axis_type($name, $name_dim)
        end

        if $def_selectdim
            NamedAxes.@def_selectdim($name, $name_dim)
        end

        if $def_eachslice
            NamedAxes.@def_eachslice($name, $name_dim)
        end
