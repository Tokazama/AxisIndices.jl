...
function _combine_axes(x::Tuple, y::Tuple)
    return (combine_axis(first(x), first(y)), _combine_axes(tail(x), tail(y))...)
end
_combine_axes(x::Tuple{}, y::Tuple) = (combine_axis(first(x), first(y)), tail(y)...)
_combine_axes(x::Tuple, y::Tuple{}) = (combine_axis(first(x), first(y)), tail(x)...)
